...
СТРАТЕГИЯ: каждый логический раздел/подраздел текста → отдельная карточка.
Для большого текста создавай 10-20+ карточек — это нормально.

Примеры хороших вопросов:
"Хронический гастрит — определение и особенности у детей"
"H. pylori — факторы патогенности"
"Факторы риска гастродуоденита — эндогенные и экзогенные"
"Патогенез H. pylori — механизм повреждения слизистой"
"Классификация гастродуоденита — по этиологии, локализации, течению, фазе"
"Антральный гастрит — клиническая картина"
"Фундальный гастрит — клиническая картина"
"Верификация H. pylori — неинвазивные методы"
"Верификация H. pylori — инвазивные методы"
"Эрадикация H. pylori — первая линия, схема, дозы"
"Эрадикация H. pylori — вторая линия, схема, дозы"
"Антисекреторные препараты при гастрите — группы, препараты, дозы"
...
1. Создай ОТДЕЛЬНУЮ карточку для КАЖДОГО раздела текста
2. Охвати АБСОЛЮТНО ВЕСЬ текст — каждый факт, цифра, доза, срок, термин
3. Все дозы (мг/кг/сут), сроки (дней), pH значения — обязательно в ответе
4. Только HTML: <b>, <ul>, <li>
5. Поле "deck" = колода::подколода от пользователя (если не указано — "Anki")
6. Верни ТОЛЬКО валидный JSON массив без пояснений:
